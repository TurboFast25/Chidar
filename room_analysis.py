...
def detect_objects_from_grid(grid, image_width, image_height, horizon_row):
    ...
    flat = [value for row in edge for value in row]
    ...
    for y in range(h):
        for x in range(w):
            if mask[y][x] and not visited[y][x]:
                ...
                if box_width >= w * 0.06 and box_height >= h * 0.06 and count >= 8:
                    ...
                    objects.append(
                        {
                            ...
                            "width": clamp((box_width / w) * 100, 3, 80),
                            "height": clamp((box_height / image_height) * 100, 3, 60),
                        }
                    )
                    if len(objects) >= 10:
                        return objects
    ...
    if brightness_grid:
        ...
        for obj in grid_objects:
            ...
            for existing in avoid_zones:
                if (
                    obj["x"] < existing["x"] + existing["width"]
                    and obj["x"] + obj["width"] > existing["x"]
                    and obj["y"] < existing["y"] + existing["height"]
                    and obj["y"] + obj["height"] > existing["y"]
                ):
                    overlaps = True
                    break
            if not overlaps:
